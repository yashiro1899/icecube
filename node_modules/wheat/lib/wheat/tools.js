...
var Helpers = {
    ...
    gravitar: function gravitar(email, size) {
        ...
        return "http://www.gravatar.com/avatar/" + hash.digest('hex') + "?r=pg&s=" + size + ".jpg&d=identicon";
    },
    ...
    decodeURIComponent: function decodeURIComponent(query) {
        return QueryString.escape(query);
    }
};
...
function render(name, data, callback, partial) {
    ...
    function loadTemplates(err, version) {
        ...
        loadTemplate(version, 'seo', this.parallel());
    ...
    function renderTemplates(err, seo, template, layout) {
        if (err) {
            callback(err);
            return;
        }
        ...
        var theme = data.article && data.article['theme'];
        theme = theme ? theme : (data.article && 'blackboard');
        ...
        data = {
            ...
            content: content,
            title: data.title || "",
            theme: theme && "/codemirror/theme/" + theme + ".css"
        };
        data.__proto__ = Helpers;
