...
var S3 = knox.createClient(auth);

function preProcessMarkdown(markdown, path) {
    ...
    markdown = markdown.replace(/s3:\/\/([0-9.\-A-Za-z]+):([0-9.\/\-A-Za-z]+)/g, function(match, bucket, name) {
        ...
        name = path + '/' + name;
        url = S3.signedUrl(name, expires);
        url = url.replace(new RegExp('//' + bucket + '.'), '//');
        url = url.replace('amazonaws.com', 'amazonaws.com/' + bucket);
        return url;
    });
...
var Data = module.exports = {
    ...
    article: Git.safe(function article(version, name, callback) {
        ...
        function(err, markdown) {
            ...
            props = preProcessMarkdown(markdown, '/articles/' + name);
            if (props.author) {
    ...
    fullArticle: Git.safe(function fullArticle(version, name, callback) {
        ...
        function finish(err, articles, log, snippets) {
            ...
            if (Object.keys(log).length > 0) {
                ...
                for (; i < 5 && i < Object.keys(log).length; i += 1) {
                    key = Object.keys(log)[i];
                    article.log[key] = log[key];
                }
            }
            ...
            articles.forEach(function(otherArticle) {
                if (otherArticle.author.name === article.author.name && otherArticle.name !== article.name) {
    ...
    author: Git.safe(function author(version, name, callback) {
        ...
        function process(err, markdown) {
            ...
            return preProcessMarkdown(markdown, '/authors/' + name);
        },
